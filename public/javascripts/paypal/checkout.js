...
(function () {
    ...
    paypal.checkout = function () {
        ...
        function _setUpPayPalBridge() {
            ...
            iframe.src = domain + '/webapps/hermes/bridge';
        ...
        function _getBtnContainers(el, result) {
            ...
            if (el.constructor.toString().indexOf('Array') > -1 || ((el.length > 0) && (typeof el.item !== 'undefined'))) {
                ...
                if(domEl) {
                    ...
                    _log("IC_SETUP_CONTAINER_ERROR: Can't find element " + el);
                }
            }
        }

        function _addButtonElement (container, btnType, clickFn, condFn) {
        ...
        function _initWithButtonGeneration () {
            ...
            //for IE8 and IE9
            if(requireBtnJs && this.onreadystatechange && !(this.readyState === 'loaded' || this.readyState === 'complete')){
                return;
            }

            if(btnsConfigList && btnsConfigList.length) {
                for(var i in btnsConfigList) {
                    ...
                    if(btnConfig.container) {
                        _addButtonElement(elDom, btnConfig.type || 'checkout', btnConfig.click, btnConfig.condition);
                ...
                for(var i in btnContainers) {
                    if(customBtnIds) {
                        btnList.push(_getBtnObject(btnContainers[i], clickFn , condFn));
                    } else {
                        _addButtonElement(btnContainers[i], jsBtnTypes[i] || 'checkout', clickFn, condFn);
                    }
                }
            }
        ...
        function _requireLoadButtonJS() {
            if(paypal.button && paypal.button.create) {
                return false;
            }
